...
// Async functions that chain to return "[name] is [age] years old" after awaiting a delay
...
async function main(name, age, delay) {
    //Add your code below
    await returnValueAfterDelay(delay, name);

    return `${name} is ${age} years old.`;
}

// Function uses a calback to check if number is even then returns a new array of even numbers based on the true or false
// response of the calback
...
const filterEvens = (array, callback) => {
    let newArr = [];
...
// Function called delay that will be passed an integer representing time in milliseconds and returns a Promise that resolves after
// that amount of time.
...
// Function that returns a diameter give a radius and logs it in a object called circle with chain of callback
...
// Function uses apply between two functions to get sum
